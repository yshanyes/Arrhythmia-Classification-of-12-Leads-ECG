...
from keras.layers import Conv1D, BatchNormalization, Activation, AveragePooling1D, Dense,GlobalMaxPooling1D,MaxPooling1D
...
class Net(object):
    ...
    def __DenseNet(
        ...
        x = MaxPooling1D(int(x.shape[1]), int(x.shape[1]))(x)
        
        cnnout = Dropout(0.2)(x)
        x1 = Bidirectional(CuDNNLSTM(62, input_shape=(x.shape[1],62),return_sequences=True,return_state=False))(cnnout)#CuDNNGRU  CuDNNLSTM

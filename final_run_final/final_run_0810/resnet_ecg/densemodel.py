...
from keras.layers import Conv1D, BatchNormalization, Activation, AveragePooling1D, Dense,GlobalMaxPooling1D,MaxPooling1D,SeparableConv1D#,DepthwiseConv1D
...
class Net(object):
    ...
    def __DenseNet(
        ...
        x = Convolution1D(filters=growth_rate, kernel_size=32, padding='same',strides=1,
                          use_bias=False, kernel_regularizer=l2(weight_decay))(inp)#(img_input)  Convolution1D SeparableConv1D
        ...
        x = AveragePooling1D(int(x.shape[1]), int(x.shape[1]))(x) #MaxPooling1D(int(x.shape[1]), int(x.shape[1]))(x)  # 
        ''' '''
    ...
    @staticmethod
    def __convolution_block(x, nb_channels, dropout_rate=0.2, bottleneck=False, weight_decay=1e-4):
        """
        ...
        x = Convolution1D(nb_channels, 32, padding='same', use_bias=False,kernel_regularizer=l2(weight_decay))(x)
    ...
    @staticmethod
    def __transition_layer(x, nb_channels, dropout_rate=0.2, compression=1.0, weight_decay=1e-4):
        """
        ...
        x = AveragePooling1D(2,2)(x) #MaxPooling1D(2,2)(x) # 
        #
        return x
    ...
    @staticmethod
    def nnet(inputs, keep_prob, num_classes, attention):
        ...
        for i in range(int(len(inputs))):
            ...
            bch = Net.__DenseNet(inp=ld,nb_classes=num_classes,depth=10,dropout_rate=0.1,growth_rate=16,compression=0.5)
            branches.append(bch)
        features = Concatenate(axis=1)(branches)#features = concatenate(branches,axis=1);print(features)#features = Flatten()(features)
	...
	'''
        ...
        if attention == True:
            ...
            x1 = Bidirectional(CuDNNLSTM(62, input_shape=(10,62),return_sequences=True,return_state=False))(cnnout)#CuDNNGRU  CuDNNLSTM
